...
class Command {
	...
	hasPermission(message, ownerOverride = true) {
		...
		if(this.ownerOnly && (ownerOverride || !this.client.isOwner(message.author))) {
			return `\`${this.name}\` adlı komutu sadece yapımcım kullanabilir.`;
		}
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				if(missing.length === 1) {
					return `\`${this.name}\` adlı komutu kullanmak için "${permissions[missing[0]]}" adlı yetkiye sahip olman gerekiyor.`;
				}
				return oneLine`
