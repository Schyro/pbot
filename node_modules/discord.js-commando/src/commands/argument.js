...
class Argument {
	...
	async obtain(msg, value, promptLimit = Infinity) {
		...
		while(!valid || typeof valid === 'string') {
			...
			//msg.reply(msg.channel.send(embed info))

			// Prompt the user for a new value
			prompts.push(await msg.channel.send(new Discord.RichEmbed().setColor('RANDOM').setDescription(`${empty ? this.prompt : valid ? valid : `You provided an invalid ${this.label}. Please try again.`}`).setFooter(`İptal Etmek İçin "iptal" Yazın. ${wait ? `Komut ${this.wait} Saniye İçinde Otomatik İptal Edilecektir.` : ''}`)
			));
